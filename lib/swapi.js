...
export async function getPlanetWithId(id) {
    return axios.get(baseUrl + 'planets/' + id).then(res => res.data);
}
...
export async function searchResource(resource, query) {
    return axios.get(baseUrl + resource +'/?search=' + query).then(res => res.data.results);
}
